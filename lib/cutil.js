...
/**
	条件1.	成对出现【 】的
	条件2.	出现 “XXX 作者 名字”
	条件3.  出现 某网XX月XX日电/讯/消息
	条件4.	出现 新浪财经讯/日电/日讯/日消息/报讯/XX网 [记者 XXX]		
**/
var cutCondition         = [/【.+?】([^\d]{0,2}记者[^\d]{2,3}[^\d ]{0,3}){0,1}/g];
		cutCondition.push(/[^\d]{0,2}本报[^\d]{2,5}讯/g);
		cutCondition.push(/[^\d\(]{0,8}作者[ ：:]{0,1}[^\d]{2,4}/g);
		cutCondition.push(/[^\n]{0,8}(([^\d \(]{2}(\([^\)]+\)){0,1}(\d{2,4}年){0,1}\d{1,2}月\d{1,2}日([电讯]|消息){0,1})|新浪财经讯|日([电讯]|消息)|报讯|[^\d \(]{2,4}网 )([^\d]{0,2}记者[^\d]{2,3}[^\d ]{0,3}){0,1}[ ：:]{0,1}(\([^\)]+\)){0,1}/g);

var pickCondition        = /\([^\)]+[记者|编辑][^\)]+\)/g;
var spaceCondition1	     = /\([^\)]+\)|报道|记者|编辑|导报/g;

var spaceSplitCoondition =/ |发自|、/g;
...
var firstContent=function(content){
	...
	//是否有空格出现，如果空格之间的文字长度小于5则扔掉		      			
	var spaceContent = content.split(spaceSplitCoondition);
	if(spaceContent.length>1){
		...
		if(iCount==spaceContent.length){
			content = content.replace(spaceSplitCoondition,"");
		}
	}
	return content;
};
...
exports.checkContent=function(rTitle,rContent){
	var content = rContent.replace(/（/g,"(").replace(/）/g,")").replace(/[ \t 　]{1,}/g," ").replace(/－{1,}/,"－").replace(/\(微博\)/g,"").split("\n"); //数据使用换行为段落分割条件 
	var _content = ["【"+rTitle+"】"];
	var _isFirst = true;
