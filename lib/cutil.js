...
var cutCondition     = /【.+?】|[^\d\(]{0,8}作者[ ：:]{0,1}[^\d]{2,4}|[^\d\(]{0,8}(([^\d \(]{2}(\d{2,4}年){0,1}\d{1,2}月\d{1,2}日([电讯]|消息){0,1})|新浪财经讯|日([电讯]|消息)|报讯)[ ：:]{0,1}(\([^\)]+\)){0,1}/g;
...
exports.checkContent=function(rTitle,rContent){
	var content = rContent.replace(/[ \t 　]{1,}/g," ").replace(/－{1,}/,"－").replace(/\(微博\)/g,"").split("\n"); //数据使用换行为段落分割条件 .replace(/\([^\)]+\)/g,"")
	var _content = ["【"+rTitle+"】"];
