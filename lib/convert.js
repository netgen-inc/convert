...
var Convert = function(settings){	
	...
	var checkRepeatContent=function(content){
		for(var i=0;i<contentHistory.length;i++){
		...
		cmysql.loadArticleContent(articleId,function(err,results){
	      ...
	      _self.emit('task-error',task);
	      	...
	      	if(_rContent!=null && _rContent.length>0){
		      	_self.emit('micro-analyse',{task:task,aticleId:aticleId,url:results['url'],title:results['title'],code:results['stock_code'],content:_rContent});
	      	}else{
	...
	_self.on('micro-analyse',function(micro){
		//micro.title,micro.code,micro.content,micro.task;
			...
			if(microContent!=""){	
				if(checkRepeatContent(microContent.substring(micro.title.length+2))){
					_logger.info(["Content-Repeat",micro.aticleId,micro.title]);
					_self.emit('task-finished',micro.task);	
				}else{
					_self.emit('micro-save',{task:micro.task,aticleId:micro.aticleId,code:micro.code,content:microContent+micro.url});
				}
			}else{
				_self.emit('task-finished',micro.task);	
			}
	...
	_self.on('micro-save',function(micro){
		cmysql.saveMicroContent(micro,function(err,_id){
			if(err){
				_logger.info(["Mysql-Err",micro.aticleId,err]);
	      _self.emit('task-error',micro.task);
			}else{
				_logger.info(["Add-Micro",micro.aticleId,micro.code,_id]);
				if(_id>0){
