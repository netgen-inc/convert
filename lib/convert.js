...
var Convert = function(settings){
	...
	_self.on('article-load', function(task,articleId,tabname){
		_logger.info([articleId,tabname]);
		console.log(articleId,tablename);
		cmysql.loadArticleContent(articleId,tablename,function(err,results){
			if (err || !results) {
