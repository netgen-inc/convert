...
var Convert = function(settings){
	...
	_self.on('article-load', function(task,articleId,tabname){
		cmysql.loadArticleContent({articleId:articleId,tabname:tabname},function(err,results){
			...
			if(meta.type==='article' && _rContent!==null && _rContent.length>0){
				...
				_logger.info(["Record-Err","Empty Content",articleId,meta.type]);
				_self.emit('task-finished',task);
			}
		}
		});
	...
	_self.on('micro-analyse',function(micro){
		//micro.title,micro.code,micro.content,micro.task;
			...
			if(microContent!==""){
				_self.emit('micro-save',{task:micro.task,articleId:micro.articleId,code:micro.code,url:micro.url,source:micro.source,content:microContent,content_type:micro_blog_type});				
			}else{
		...
		cmysql.saveMicroContent(micro,function(err,_id){
			if(err){
				...
				if(_id>=0){
					if(_id>0){
						var _meta = "";
						if(micro.content_type==='zixun'){
							_meta = "&meta="+_id;
						}
						request({ uri:urlChange+encodeURIComponent(micro.url)+_meta, timeout:5000 }, function (error, response, body) {
							if(error || response.statusCode != 200) {
								console.log(["request-error",urlChange,micro.url,_id]);
							}else{
								if(body){
									var jdata=JSON.parse(body);
									if(jdata && jdata.data && jdata.data.url){
										var regionUrl = jdata.data.url;
										_logger.info(["request-micro-shortUrl",_id,regionUrl,"[",_meta,"]"]);
										cmysql.saveMicroContentUrl(_id,regionUrl,function(err,blogid){
											if(err){
													_logger.info(["Err-Micro-Url",_id,regionUrl]);
											}
										});
									}else{
										_logger.info(["Err-Change-shortUrl",jdata,_id]);
									}
								}else{
									_logger.info(["Err-Change-shortUrl",_id,body]);
								}
							}
