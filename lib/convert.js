		...
		cmysql.loadArticleContent(articleId,function(err,results){
	      ...
	      _self.emit('task-error',task);
	      	...
	      	if(_rContent!=null && _rContent.length>0){
		      	_self.emit('micro-analyse',{task:task,articleId:articleId,url:results['url'],title:results['title'],code:results['stock_code'],content:_rContent});
	      	}else{
		...
		//micro.title,micro.code,micro.content,micro.task;
			...
			if(microContent!=""){	
				if(checkRepeatContent(microContent.substring(micro.title.length+2))){
					_logger.info(["Content-Repeat",micro.articleId,micro.title]);
					_self.emit('task-finished',micro.task);	
				}else{
					_self.emit('micro-save',{task:micro.task,articleId:micro.articleId,code:micro.code,content:microContent+micro.url});
				}
			}else{
				_self.emit('task-finished',micro.task);	
			}
