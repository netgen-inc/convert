	...
	_self.on('article-load', function(task,articleId){
		cmysql.loadArticleContent(articleId,function(err,results){
			...
			if(_rContent!=null && _rContent.length>0){
				self.emit('micro-analyse',{task:task,articleId:articleId,url:results['url'],title:results['title'],code:results['stock_code'],source:results['source'],content:_rContent});
			}else{
