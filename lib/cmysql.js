var CMysql = function(settings){	
  ...
  var mysql = require('mysql'), 
	...
	this.saveMicroContent=function(micro,cb){
		mysqlw.query("insert into micro_blog(article_id,content_type,stock_code,source,in_time,url,content)values(?,1,?,?,UNIX_TIMESTAMP(),?,?)",[micro.articleId,micro.code,micro.source,micro.url,micro.content],function(err, results, fields) {
			var _id = -1;
