var CMysql = function(settings){
  ...
  var mysql = require('mysql'),
	...
	this.saveArticleTopic=function(micro,cb){
		mysqlw.query("insert into article_topic(articleid,stockcode,boardname,in_time)values(?,?,?,UNIX_TIMESTAMP())",[micro.articleid,micro.stockcodes,micro.boardname],function(err, results, fields) {
			if(err){
				cb(err,-1);
			}else{
				var _id = -1;
